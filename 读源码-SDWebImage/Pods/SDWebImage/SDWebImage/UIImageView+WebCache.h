 ...
 * This can be done by using `sd_setImageWithURL:` with `SDWebImageQueryDiskSync`. But take care that if the memory cache missed, query disk cache synchronously may reduce the frame rate  ✨同步的去访问硬盘缓存有可能会掉帧
 *
 * The download is asynchronous and cached.
 *
 * @param url            The url for the image.
