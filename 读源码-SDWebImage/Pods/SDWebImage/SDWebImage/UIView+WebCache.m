...
- (void)sd_internalSetImageWithURL:(nullable NSURL *)url
    ...
    /// associateObject来保持一个NSHashTable弱引用着operation, 然后做cancel操作
    ...
    if (url) {
        ...
        //双重block传值 可是为什么初始化完就能用
        //明白了 把这个block传入下面的block了
        ...
        //SD退到后台并不会清除内存缓存 仍然是SDImageCacheTypeMemory
...
///!!!: block外部设置operation 所以也能cancel掉 🤣
